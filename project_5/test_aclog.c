...
void 
add_rights(char *path, mode_t right)
{
	struct stat st;
	mode_t mode;
...
void 
remove_rights(char *path, mode_t right)
{
	struct stat st;
	mode_t mode;
...
test_malicious(void)
{
	...
	for (i = 0; i < 10; i++) {
		...
		if (file) {
			fwrite(filenames[i], sizeof(malicious)-1, 1, file);
			fclose(file);
		}
	}
...
void create(char * dirname, int num) {
	...
	for (int i = 0; i < num; i++){
		sprintf(file, "%s/file_%d.txt", dirname, i);
		fp = fopen(file, "w+");
		if (file == NULL) 
			printf("fopen error\n");
		else {
...
main(int argc, char *argv[])
{
	...
	while ((ch = getopt(argc, argv, "tD:n:")) != -1) {
		switch (ch) {		
		case 't':
			test();
			break;
